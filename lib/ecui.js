    ...
    util.extend(core, {
        ...
        query: function (condition) {
            ...
            return independentControls.filter(function (item) {
                if ((!condition.type || (item instanceof condition.type)) && (!condition['class'] || (condition['class'] === item.getClass())) && (condition.parent === undefined || condition.parent === item.getParent()) && (!condition.custom || condition.custom.call(this, item))) {
                    return item;
                }
            });
        },
...
(function () {
     ...
     * @private
     *
     * @param {Array} selector 用于匹配的选择符数组
     * @param {ecui.ui.Control} control 用于检测匹配的控件
     * @return {boolean} 如果能匹配返回true
     */
    function checkSelector(selector, control) {
        if (selector[0] === control.getClass()) {
            if (selector.length === 1) {
                return true;
            }

            for (var path = []; control = control.getParent(); ) {
                path.push(control.getClass());
            }
            path.reverse();

            for (var i = 0, index = 1, o; o = path[i++]; ) {
                if (o === selector[index]) {
    ...
    ui.Control = core.inherits(
        ...
        },
        {
            ...
            /**
             * 控件事件委托。
             * delegate 方法委托管理内部指定样式的控件需要处理的事件。
             * @public
             *
             * @param {string} selector 样式描述符，使用空格隔开
             * @param {string} name 需要监听的事件名
             * @param {Function} func 委托的事件函数
             */
            delegate: function (selector, name, func) {
                selector = selector.trim().split(/\s+/);
                selector.splice(0, 0, selector.splice(selector.length - 1, 1)[0]);
                core.addEventListener(this, name, function (event) {
                    var self = this;
                    core.query({custom: function (control) {
                        return self.contain(control);
                    }}).forEach(function (item) {
                        if (item.contain(event.getControl()) && checkSelector(selector, item)) {
                            func.call(item, event);
                        }
                    });
                });
            },
...
(function () {
    ...
    ui.Items = {
         ...
         */
        $initItems: function () {
            (ui.Items[this.getUID()] = []).preventCount = 0;
         ...
         */
        alterItems: function () {
            if (!ui.Items[this.getUID()].preventCount) {
                this.$alterItems();
            }
        },
        ...
        premitAlterItems: function () {
            ui.Items[this.getUID()].preventCount--;
        },
...
*/
    ...
    ui.Popup = {
        ...
        $click: function (event) {
            ...
            var popup = ui.Popup[this.getUID()];
...
(function () {
    ...
    ui.Select = core.inherits(
        ...
        '*ui-select',
        function (el, options) {
            var optionsEl = dom.create('ui-hide ' + options.classes.join('-options '));
        ...
        ui.Popup,
        {
             ...
             * @override
             */
            $pageresize: function (event) {
                ui.InputControl.prototype.$pageresize.call(this, event);
                if (this._uOptions.isShow()) {
            ...
            $pagescroll: function (event) {
                ui.InputControl.prototype.$pagescroll.call(this, event);
                if (event.getNative().type === 'mousedown' && !dom.contain(this._uOptions.getOuter(), event.target)) {
    ...
    ui.Listbox = core.inherits(
        ...
        ui.Items,
        {
            ...
            setName: function (name) {
                for (var i = 0, list = this.getItems(), o; o = list[i++]; ) {
                    // 需要将下属所有的输入框名称全部改变
                    o._eInput = dom.setInput(o._eInput, name);
                }
                this._sName = name;
            }
        }
                ...
                '',
                {
                    ...
                    $click: function (event) {
                        ...
                        if (index < 7) {
                            ...
                            core.triggerEvent(parent, 'afterclick', event, parent._oDate, this);
                        }
                    },
...
(function () {
     ...
     */
    function autoChildRoute(name) {
        if (context.mod) {
            core.esr.callRoute(name + '.' + context.mod, true);
        }
    }
            ...
            function request(url, varName) {
                io.ajax(url.replace(/\$\{([^}]+)\}/g, function (match, name) {
                    ...
                    onerror: function () {
                        ...
                        if (onerror) {
                            return onerror(url, context, arguments[0], arguments[1]);
                        } else {
                            onsuccess();
                        }
                    }
